...
//	Init_Sport_Interrupts();
	...
	while (1)
	{
		while(EthPacketRecv(&RecvData, packetBuffer, DATA_SIZE))
		{
			// Decode
		    if(0 == ping) 
			{
				_decode_mcvsd((fract16*)RecvData.voiceData,TxPing+0,FRAMESIZE, delta0, delta_min, alpha, beta, &s1_rx_L, &s2_rx_L, &sr_rx_L, &delta_rx_L, &d_rx_L,2, &s3_rx_L);
				_decode_mcvsd((fract16*)RecvData.voiceData+BIT_OUTPUT_SIZE,TxPing+1,FRAMESIZE, delta0, delta_min, alpha, beta, &s1_rx_R, &s2_rx_R, &sr_rx_R, &delta_rx_R, &d_rx_R,2, &s3_rx_R);	
			}
			else
			{
				_decode_mcvsd((fract16*)RecvData.voiceData,TxPong+0,FRAMESIZE, delta0, delta_min, alpha, beta, &s1_rx_L, &s2_rx_L, &sr_rx_L, &delta_rx_L, &d_rx_L,2, &s3_rx_L);
				_decode_mcvsd((fract16*)RecvData.voiceData+BIT_OUTPUT_SIZE,TxPong+1,FRAMESIZE, delta0, delta_min, alpha, beta, &s1_rx_R, &s2_rx_R, &sr_rx_R, &delta_rx_R, &d_rx_R,2, &s3_rx_R);	
			}
			ping ^= 0x1;
			mdelay(10);
		}
