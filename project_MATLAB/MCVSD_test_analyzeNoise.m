...
LPF_CVSD=hamming(len_LPF_CVSD)/(2^4); %hamming窗
LPF_MCVSD1=hamming(len_LPF_MCVSD1)/(2^4); %hamming窗
LPF_MCVSD2=hamming(len_LPF_MCVSD2)/2; %hamming窗


%语音信号相关参数___________________________________________________________
x=0:255;
voice_in=zeros(1,1024);
voice_in(x+1)=sin(2*pi*500/48000*x);
%voice_in=wavread('voice_48kHz_16bit.wav'); %输入信号，单声道，48kHz，16bit量化
...
%测试用变量定义_____________________________________________________________
d_record=zeros(1,L);
delta_mod_record=zeros(1,L);
p_record=zeros(1,L);
sp_record_tx=zeros(1,L);
sp_record_rx=zeros(1,L);
...
for m=0:num_frame-1
    ...
    if ping==0
        ...
        for n=1:framesize;
            ...
            delta_mod_tx=(all(buffer_al_tx)+~any(buffer_al_tx))*delta0*p_tx*32;
            delta_tx=beta*delta_tx+delta_mod_tx+delta_min;
            delta_signed_tx=delta_tx*(2*d_tx(n)-1);
            
            %记录测试数据
            sp_record_tx(m*framesize+n)=sp_tx;
            p_record(m*framesize+n)=p_tx;
            delta_mod_record(m*framesize+n)=delta_mod_tx;
    ...
    if ping==0        
        for n=1:framesize
            ...
            delta_mod_rx=(all(buffer_al_rx)+~any(buffer_al_rx))*delta0*p_rx*32;
            delta_rx=beta*delta_rx+delta_mod_rx+delta_min;
            delta_signed_rx=delta_rx*(2*d_rx(n)-1);
            
%             sp_record_rx(m*framesize+n)=sp_rx;
...
% figure;
% hold on;
% plot(t,voice_in(t),'b'),stem(t,sp_record_tx(t),'r');
% plot(t,p_record(t),'g');
% 
% figure;
% hold on;
% plot(t,delta_mod_record(t));

figure;
hold on;
plot(t,voice_in(t),'b',t,voice_out(t),'r');
legend('输入波形','输出波形');
xlabel('time/sample'),ylabel('magnitude');
title('输入波形与重建波形的对比');
